const VERSION='v37-colleague-calendar-test';
...
<div class="grid"><main>${wavePanel(plogs)}${logsPanel(logs)}</main><aside>${onboardingPanel()}${dailyPanel()}${weeklyPanel(plogs)}${reminderPanel()}${settingsPanel()}${calendarTestPanel()}${qaPanel()}${devPanel()}</aside></div>${state.quick?quickHtml():''}${state.toast?`<div class="toast">${esc(state.toast)}</div>`:''}</div>`}
...
async function connectGoogleCalendar(){const clientId=(state.settings.googleClientId||EMBEDDED_GOOGLE_CLIENT_ID||'').trim();if(!clientId){toast('OAuth Client IDが未設定です');return}try{toast('Google認可を準備しています');await loadGoogleIdentity();const tokenClient=google.accounts.oauth2.initTokenClient({client_id:clientId,scope:'https://www.googleapis.com/auth/calendar.readonly',prompt:'consent',callback:async(resp)=>{if(resp.error){console.warn(resp);toast(resp.error==='access_denied'?'Google認可がキャンセルされました':'Google認可でエラーが出ました');return}try{const events=await fetchTodayCalendarEvents(resp.access_token);state.calendarEvents=events;events.forEach(ev=>{if(!state.tasks.includes(ev.label))state.tasks.unshift(ev.label)});state.tasks=[...new Set(state.tasks)].slice(0,50);state.settings.calendarConnected=true;toast(events.length?`${events.length}件の予定を候補にしました`:'今日の予定は見つかりませんでした');render()}catch(e){console.error(e);toast(String(e.message||e).includes('403')?'予定取得権限を確認してください':'予定取得に失敗しました')}}});tokenClient.requestAccessToken()}catch(e){console.error(e);toast(String(e.message||e).includes('origin')?'Google CloudのOrigin設定を確認してください':'Google連携の準備に失敗しました')}}
...
async function setupSW(){if(!('serviceWorker'in navigator))return;try{state.sw=await navigator.serviceWorker.register('/Namilog/namilog-sw.js?v=37');state.sw.update?.();navigator.serviceWorker.addEventListener('message',e=>{if(e.data?.type==='OPEN_QUICK_CHECK'){resetDraft();state.quick=true;render()}})}catch(e){}}
...
async function showNotif(reason){if(!('Notification'in window)||Notification.permission!=='granted'){toast('先に通知を許可してね');return}try{const reg=state.sw||await navigator.serviceWorker.ready;await reg.showNotification('NamiLog｜今の波をそっと残そう',{body:reason==='scheduled'?'感情・進み具合・いまのタスクを、そっと記録する時間です。':'クリックするとQuick Checkを開きます。',tag:'namilog-v37',renotify:true,data:{url:'/Namilog/?quickCheck=1'},actions:[{action:'open',title:'記録する'},{action:'later',title:'あとで'}]});if(reason==='manual')toast('通知を出しました')}catch{new Notification('NamiLog｜今の波をそっと残そう',{body:'クリックするとQuick Checkを開きます。'});toast('通知を出しました')}}
